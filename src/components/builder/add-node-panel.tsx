      ...
      <AnimatePresence mode="wait" initial={false}>
        {!open && (
          <motion.button
            ...
            className="absolute left-3 top-3 z-sticky flex items-center gap-1.5 rounded-lg bg-white border border-border px-3 py-2 text-sm font-medium text-text shadow-e1 hover:bg-border-light transition-colors"
          >
        ...
        {open && (
          <motion.div
            ...
            className="absolute left-0 top-0 z-sticky h-full w-56 bg-white border-r border-border shadow-e3 overflow-y-auto"
          >
            <div className="flex items-center justify-between px-3 py-2.5 border-b border-border-light">
              <span className="text-sm font-semibold text-text">Add Node</span>
              <button onClick={onToggle} className="p-0.5 rounded hover:bg-border-light">
                <X className="h-4 w-4 text-text-faint" />
              </button>
            </div>

            <div className="p-2">
              <p className="text-[10px] font-medium text-text-faint uppercase tracking-wider px-1 mb-1">
                Page-tier
              </p>
              {pageTier.map((item) => (
                <div
                  ...
                  className="flex items-center gap-2 px-2 py-1.5 rounded-md cursor-grab hover:bg-border-light active:cursor-grabbing text-sm text-text"
                >
              ...
              <div className="mt-3">
                ...
                {contentTier.map((item) => (
                  <div
                    ...
                    className="flex items-center gap-2 px-2 py-1.5 rounded-md cursor-grab hover:bg-border-light active:cursor-grabbing text-sm text-text"
                  >
                    <span className="h-5 w-5 rounded bg-warning-subtle flex items-center justify-center text-[10px] text-warning-strong font-bold">
                      {item.label[0]}
                    </span>
                    {item.label}
                  </div>
