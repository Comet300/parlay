...
export function SingleChoiceEditor({ nodeId }: { nodeId: string }) {
  const node = useBuilderStore((s) => s.nodes.find((n) => n.id === nodeId))
  const updateNodeData = useBuilderStore((s) => s.updateNodeData)

  if (!node || node.data.type !== 'single_choice') return null
  const d = node.data
  ...
  return (
    <div className="p-3">
      <h3 className="text-sm font-semibold mb-3">Single Choice</h3>
