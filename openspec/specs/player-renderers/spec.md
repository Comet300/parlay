...
minLabel (left) and maxLabel (right), from min to max values.
Selecting a value SHALL immediately record it in the session responses
(keyed by the node's slug).
The selected value SHALL be visually highlighted.
...
On mount, the client SHALL send an init request to /api/llm-proxy with
form_id, facet_id, node_slug, and visitor_id.
The server returns the conversation_id and first bot message (pre-warmed).
The client displays the first bot message as the opening chat bubble.
Each subsequent user message SHALL be POSTed to /api/llm-proxy with the
conversation_id and message.
The client SHALL NOT send setup_prompt, ending_condition, or system context.
The response SHALL stream back and render as a chat bubble.
When the server sends ended: true:
- The text input SHALL be disabled (user can no longer type)
- A Continue button SHALL appear at the bottom of the chat
- The system SHALL smooth-scroll to the Continue button
maxTurns is the hard fallback limit (server-enforced).
