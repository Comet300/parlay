...
## Purpose
Define the foundational project structure, dependencies, configuration files,
authentication wiring, and Supabase client setup for the Parlay application.
...
The project SHALL include TailwindCSS v4 configured via the `@tailwindcss/vite` plugin in `vite.config.ts`. A `src/styles/globals.css` file SHALL include `@import "tailwindcss"`. The root layout SHALL import this stylesheet.
